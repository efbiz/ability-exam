...
import org.efbiz.ability.exam.common.util.StringUtil;
import org.efbiz.ablility.exam.manager.persistence.ExamMapper;
...
public class ExamServiceImpl implements ExamService {
    ...
    try {
      ...
      if (exam.getGroupIdList() != null && exam.getGroupIdList().size() > 0) {
        ...
        for (User user : userList) {
          ...
          String seriNo = sdf.format(now) + StringUtil.format(user.getUserId(), 3) + StringUtil
              .format(exam.getExamId(), 3);
  ...
  public void addExamUser(int examId, String userNameStr, HashMap<String, Role> roleMap) {
    ...
    try {
      ...
      List<User> userList = userMapper
          .getUserByNames(userNames, roleMap.get("ROLE_STUDENT").getRoleId());
      Exam exam = examMapper.getExamById(examId);
      ExamPaper examPaper = examPaperMapper.getExamPaperById(exam.getExamPaperId());
      SimpleDateFormat sdf = new SimpleDateFormat("yyMMdd");
      Date now = new Date();
      for (User user : userList) {
        ExamHistory history = this
  ...
  @Override
  public List<ExamHistory> getUserExamHistListByExamId(int examId, String searchStr, String order,
      int limit, Page<ExamHistory> page) {
    // TODO Auto-generated method stub
