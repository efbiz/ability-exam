...
import org.efbiz.ability.exam.common.domain.exam.Exam;
import org.efbiz.ability.exam.common.domain.exam.ExamHistory;
...
import org.efbiz.ability.exam.common.util.QuestionAdapter;
...
import org.efbiz.ablility.exam.manager.service.UserService;
...
public class ExamPageTeacher {
  ...
  private String examListPage(Model model, HttpServletRequest request,
    ...
    String pageStr = PagingUtil
        .getPagelink(page, pageModel.getTotalPage(), "", "teacher/exam/exam-list");
  ...
  private String modelTestListPage(Model model, HttpServletRequest request,
      @RequestParam(value = "page", required = false, defaultValue = "1") int page) {
  ...
  private String examAddPage(Model model, HttpServletRequest request) {
    ...
    List<ExamPaper> examPaperList = examPaperService
        .getEnabledExamPaperList(userInfo.getUsername(), null);
  ...
  private String examStudentListPage(Model model, HttpServletRequest request,
    ...
    List<ExamHistory> histList = examService
        .getUserExamHistListByExamId(examId, searchStr, order, limit, pageModel);
    String pageStr = PagingUtil.getPageBtnlink(page,
        pageModel.getTotalPage());
    List<Group> groupList = userService.getGroupListByUserId(userInfo.getUserid(), null);
    model.addAttribute("groupList", groupList);
  ...
  private String studentAnswerSheetPage(Model model, HttpServletRequest request,
      @PathVariable int histId) {
    ...
    if (examPaper.getContent() != null && !examPaper.getContent().equals("")) {
      ...
      List<QuestionQueryResult> questionList = gson
          .fromJson(content, new TypeToken<List<QuestionQueryResult>>() {
          }.getType());
  ...
  private String markExampaperPage(Model model, HttpServletRequest request,
      @PathVariable int examhistoryId) {
