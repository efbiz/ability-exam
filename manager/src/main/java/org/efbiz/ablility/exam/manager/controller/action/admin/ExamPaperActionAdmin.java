package org.efbiz.ablility.exam.manager.controller.action.admin;
...
import org.efbiz.ability.exam.common.domain.exam.PaperCreatorParam;
...
public class ExamPaperActionAdmin {
  ...
  public @ResponseBody
  List<QuestionQueryResult> getQuestion5add(Model model, HttpServletRequest request,
  ...
  Message exampaperOnUpdate(Model model,
    ...
    try {
      ...
      List<QuestionQueryResult> questionList = questionService
  ...
  Message createDocPaper(@PathVariable("examPaperId") int examPaperId) {
    ...
    try {
      ...
      msg.setMessageInfo(
          (relativePath + "," + examPaper.getName() + ".docx").replace(",", File.separator));
    } catch (Exception e) {
