...
public class ExamPaperPageTeacher {
  ...
  private String examPaperListPage(Model model, HttpServletRequest request,
    ...
    String pageStr = PagingUtil
        .getPagelink(page, pageModel.getTotalPage(), "",
            "teacher/exampaper/exampaper-list/" + paperType);
  ...
  private String examPaperEditPage(Model model, HttpServletRequest request,
    ...
    if (examPaper.getContent() != null && !examPaper.getContent().equals("")) {
      ...
      List<QuestionQueryResult> questionList = gson
          .fromJson(examPaper.getContent(), new TypeToken<List<QuestionQueryResult>>() {
          }.getType());
  ...
  private String examPaperPreviewPage(Model model, HttpServletRequest request,
      @PathVariable int examPaperId) {
    String strUrl = "http://" + request.getServerName() // 服务器地址
        + ":" + request.getServerPort() + "/";
