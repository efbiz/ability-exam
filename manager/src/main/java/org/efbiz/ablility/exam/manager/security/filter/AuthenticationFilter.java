package org.efbiz.ablility.exam.manager.security.filter;
...
public class AuthenticationFilter extends UsernamePasswordAuthenticationFilter {
  ...
  public Authentication attemptAuthentication(HttpServletRequest request,
    ...
    if (!passwordEncoder.matches(userDetails.getPassword(), result) || "0"
        .equals(userDetails.getEnabled()) || userDetails == null) {
      //System.out.println("用户名或密码错误！");
      throw new AuthenticationServiceException("用户名或密码错误！");
    }
    if (!userDetails.getRolesName().contains("ROLE_ADMIN") && !userDetails.getRolesName()
  ...
  protected void checkValidateCode(HttpServletRequest request) {
    ...
    if (StringUtils.isEmpty(validateCodeParameter) || !sessionValidateCode
